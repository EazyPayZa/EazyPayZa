EazyPayZa Core version 3.0.4 is now available from:

  <https://github.com/eazypayzaproject/eazypayza/releases>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/eazypayzaproject/eazypayza/issues>


Mandatory Update
==============

EazyPayZa Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zEZPAY protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
If you are running an older version, shut it down. Wait until it has completely shut down (which might take a few minutes for older versions), then run the installer (on Windows) or just copy over /Applications/EazyPayZa-Qt (on Mac) or eazypayzad/eazypayza-qt (on Linux).


Compatibility
==============

EazyPayZa Core is extensively tested on multiple operating systems using
...
EazyPayZa Core should also work on most other Unix-like systems but is not
...
- #327 `aa1235a` [Main] Don't limit zEZPAY spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zEZPAY (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when eazypayzad --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] EazyPayZa not responding on user quitting app (Aaron Langford)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/eazypayza-project-translations/).
