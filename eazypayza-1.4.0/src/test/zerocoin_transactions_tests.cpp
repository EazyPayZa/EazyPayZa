...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zphrTracker = unique_ptr<CzEZPAYTracker>(new CzEZPAYTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZEZPAY_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZEZPAY_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
