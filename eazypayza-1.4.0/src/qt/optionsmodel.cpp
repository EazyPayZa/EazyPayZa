...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EZPAY);
    ...
    if (!settings.contains("nAnonymizeEazyPayZaAmount"))
        settings.setValue("nAnonymizeEazyPayZaAmount", 1000);

    nAnonymizeEazyPayZaAmount = settings.value("nAnonymizeEazyPayZaAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEazyPayZaAmount"))
        SoftSetArg("-anonymizeeazypayzaamount", settings.value("nAnonymizeEazyPayZaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eazypayza.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEazyPayZaAmount:
            return QVariant(nAnonymizeEazyPayZaAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEazyPayZaAmount:
            nAnonymizeEazyPayZaAmount = value.toInt();
            settings.setValue("nAnonymizeEazyPayZaAmount", nAnonymizeEazyPayZaAmount);
            emit anonymizeEazyPayZaAmountChanged(nAnonymizeEazyPayZaAmount);
            break;
        case CoinControlFeatures:
