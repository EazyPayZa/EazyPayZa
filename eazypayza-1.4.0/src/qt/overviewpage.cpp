...
// Copyright (c) 2017-2019 The EazyPayZa Developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::EZPAY)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sEZPAYPercentage, QString& szEZPAYPercentage)
{
    int nPrecision = 2;
    ...
    szEZPAYPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sEZPAYPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Percentage labels
    // ui->labelEZPAYPercent->setText(sPercentage);
    // ui->labelzEZPAYPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zEZPAY.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in eazypayza.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in eazypayza.conf");
    }
    ...
    bool showEZPAYAvailable = settingShowAllBalances || phrAvailableBalance != nTotalBalance;
    bool showWatchOnlyEZPAYAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showEZPAYPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyEZPAYPending = watchUnconfBalance != 0;
    bool showEZPAYLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyEZPAYLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showEZPAYAvailable || showWatchOnlyEZPAYAvailable);
    ui->labelBalanceText->setVisible(showEZPAYAvailable || showWatchOnlyEZPAYAvailable);
    ui->labelWatchAvailable->setVisible(showEZPAYAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showEZPAYPending || showWatchOnlyEZPAYPending);
    ui->labelPendingText->setVisible(showEZPAYPending || showWatchOnlyEZPAYPending);
    ui->labelWatchPending->setVisible(showEZPAYPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showEZPAYLocked || showWatchOnlyEZPAYLocked);
    ui->labelLockedBalanceText->setVisible(showEZPAYLocked || showWatchOnlyEZPAYLocked);
    ui->labelWatchLocked->setVisible(showEZPAYLocked && showWatchOnly);
    ...
    bool showzEZPAYAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzEZPAYUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzEZPAYImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    // ui->labelzBalanceMature->setVisible(showzEZPAYAvailable);
    // ui->labelzBalanceMatureText->setVisible(showzEZPAYAvailable);
    // ui->labelzBalanceUnconfirmed->setVisible(showzEZPAYUnconfirmed);
    // ui->labelzBalanceUnconfirmedText->setVisible(showzEZPAYUnconfirmed);
    // ui->labelzBalanceImmature->setVisible(showzEZPAYImmature);
    // ui->labelzBalanceImmatureText->setVisible(showzEZPAYImmature);
    //bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    //ui->labelEZPAYPercent->setVisible(showPercentages);
    // ui->labelzEZPAYPercent->setVisible(showPercentages);
