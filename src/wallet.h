...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 EZPAY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zEZPAY send
enum ZerocoinSpendStatus {
    ZEZPAY_SPEND_OKAY = 0,                            // No error
    ZEZPAY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZEZPAY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZEZPAY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZEZPAY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZEZPAY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZEZPAY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZEZPAY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZEZPAY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZEZPAY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZEZPAY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZEZPAY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZEZPAY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZEZPAY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZEZPAY_SPENT_USED_ZEZPAY = 14,                      // Coin has already been spend
    ZEZPAY_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZEZPAY_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    bool CreateZEZPAYOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzEZPAYWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzEZPAYTracker> zphrTracker;
    ...
    void setZWallet(CzEZPAYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zphrTracker = std::unique_ptr<CzEZPAYTracker>(new CzEZPAYTracker(strWalletFile));
    }

    CzEZPAYWallet* getZWallet() { return zwalletMain; }
