...
CzEZPAYWallet::CzEZPAYWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEZPAYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEZPAYSeed_deprecated()) {
                    LogPrintf("%s: Updated zEZPAY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzEZPAYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEZPAYCount(nCountLastUsed);
    else if (!walletdb.ReadZEZPAYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEZPAYWallet::Lock()
{
    seedMaster = 0;
}

void CzEZPAYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEZPAYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEZPAY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEZPAYWallet::LoadMintPoolFromDB()
{
...
void CzEZPAYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEZPAYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEZPAYWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzEZPAYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEZPAY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzEZPAYWallet::SeedToZEZPAY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEZPAYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEZPAYWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZEZPAYCount(nCountLastUsed);
...
void CzEZPAYWallet::GenerateDeterministicZEZPAY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEZPAYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEZPAYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
